...
namespace DemoDotNet.Common
{
    public class Email
    {
        ...
        public Email( IConfiguration configuration, IHostingEnvironment env)
        {
        ...
        public void ForgetPasswordMail(UserMaster userMaster)
        {
            try
            {
                ...
                string forgetPasswordUrl = _configuration["clientWebUrl"] + "reset-pasword?userid=" + CrptographyEngine.Base64Encode(userMaster.UserID.ToString());
        ...
        private async Task SendMail(string body, string subject, string toAddress)
        {
            try
            {
                ...
                {
                    mail.From = new MailAddress(emailSettings.MailFrom);
